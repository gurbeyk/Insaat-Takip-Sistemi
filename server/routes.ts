...
import { setupAuth, isAuthenticated } from "./auth"; // DÜZELTİLDİ: replitAuth değil auth
import {
  insertProjectSchema,
...
export async function registerRoutes(
  ...
  app.get("/api/auth/user", isAuthenticated, async (req: any, res) => {
    try {
      // DÜZELTİLDİ: claims.sub yerine id kullanıyoruz
  ...
  app.get("/api/projects/:id/reports", isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Get work items by unit type for material breakdown (exact match only)
